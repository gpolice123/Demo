...
#        "model_path": 'logs/000/trained_model.h5',
        "model_path": 'logs/000/trained_weights_succ.h5',
...
#        "classes_path": 'model_data/coco_classes.txt',
    ...
    def generate(self):
        model_path = os.path.expanduser(self.model_path)            #如果目录中有~或者~user的话，会替换成当前用户的目录
        assert model_path.endswith('.h5'), 'Keras model or weights must be a .h5 file.'

        # Load model, or construct model and load weights.
        num_anchors = len(self.anchors)
        num_classes = len(self.class_names)
        is_tiny_version = num_anchors==6 # default setting          #如果anchor少于6的话，使用tiny模式
        try:
            self.yolo_model = load_model(model_path, compile=False)     #只能加载原始的yolo.h5文件，不能加载自己训练的h5模型文件。如果加载会出错，走except流程
        except:
        ...
        print('eval before',self.yolo_model.output)

        boxes, scores, classes = yolo_eval(self.yolo_model.output, self.anchors,            #yolo_eval
                len(self.class_names), self.input_image_shape,
                score_threshold=self.score, iou_threshold=self.iou)
        print('eval after',self.yolo_model.output)
